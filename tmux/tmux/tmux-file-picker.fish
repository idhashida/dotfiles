...
# Проверка AI
set ai_mode false
if pgrep -P "$pane_pid" -f "opencode|claude|codex" >/dev/null
    set ai_mode true
end
...
# Выбор файлов
set selected (cd "$git_root"; fdfind --type f --hidden --follow --exclude .git | \
    fzf --multi --height 100% --color=bg:#090B10,fg:#e0def4,hl:#c4a7e7,fg+:#e0def4,bg+:#403d52,hl+:#9ccfd8,info:#6e6a86,prompt:#31748f,pointer:#ebbcba,marker:#eb6f92,spinner:#f6c177,header:#6e6a86,border:#26233a --preview "batcat --style=numbers --color=always {}")
